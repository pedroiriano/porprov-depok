...
- CRUD Master Data, pemilihan Media Library, soft delete, dan pemulihan melalui Recycle Bin Admin tersedia untuk Cabor, Nomor Pertandingan, Kontingen, City Guide, Media, Hero, Venue, dan Jadwal. City Guide memiliki koordinat terverifikasi, URL Google Maps opsional, pagination server-side Admin, form terstruktur Catering/Info Travel, serta satu pin rekomendasi utama seluruh Venue dengan default `Department Sports Lab`. Info Travel mencatat jenis/jumlah armada; gambar selalu memakai Media Library dan bukan field screenshot eksternal. Status deployment mengikuti `FEATURES.md`.
