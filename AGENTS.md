...
> **Kontrak Tahap 12:** kategori Panduan Kota adalah taksonomi dinamis; akun tidak aktif tetap terlihat dan berbeda dari soft delete; hak akses granular wajib ditegakkan pada UI, Gateway, dan service; Pengelola Lokasi hanya mendapat domain Venue; audit mengutamakan username dengan snapshot immutable; notifikasi berscope penerima; grafik Admin memakai ApexCharts lazy-load; Dropzone tidak boleh melemahkan validasi unggah; kegagalan render harus menghasilkan recovery UI. ADR-0019 dan `docs/uiux/ADMIN_ENTERPRISE_FEATURES_V12.md` adalah baseline terverifikasi.

> **Kontrak kandidat Tahap 16:** migrator `4.20.1-porprov.1`, PostgreSQL 15/PostGIS 3.5 Alpine, Prometheus `3.13.3-porprov.1`, Go builder 1.26.6, `decode-uri-component` 0.5.0, dan `uuid` 11.1.1 adalah baseline hardening lokal. Commit upstream builder wajib diverifikasi; 21 image buatan repository wajib mempunyai SBOM CycloneDX dan nol High/Critical fixable pada Trivy. Bukti lokal tidak menggantikan CodeQL/dependency review protected CI dan tidak memberi izin deployment.

> **Kontrak staging Tahap 17:** build Admin production memakai `VITE_ADMIN_CUBA_PHASE_1=true`, sementara nilai `false` tetap menjadi rollback build. Source staging wajib terpisah dari checkout aktif; 21 image linux/amd64, SBOM, Trivy, backup production lengkap, checksum, salinan terenkripsi off-host, dan latihan migrasi terisolasi wajib PASS sebelum cutover. Staging berhenti sebelum load image, swap source, restart, migrasi/bootstrap production, reload Nginx, atau perubahan data.

> **Kontrak Keycloak Peran kustom Tahap 15:** hanya confidential service account `service-account-porprov-backend-service` yang boleh menerima role client `realm-management` berupa `manage-users`, `view-users`, dan `manage-realm`. Klien browser/mobile dilarang menerima hak tersebut. Bootstrap client wajib idempotent dan secret wajib berasal dari environment yang tidak dilacak Git. UAT lokal pembuatan, perubahan status, arsip, dan pemulihan Peran kustom telah lulus serta seluruh fixture telah di-rollback; production tetap tidak berubah dan memerlukan otorisasi deployment terpisah.
...
>
> Runtime Go source, CI, dan builder container wajib minimum 1.26.6 sampai baseline diperbarui melalui advisory resmi berikutnya.
...
- Golang HTTP service wajib memakai `pgxpool` atau pool ekuivalen untuk akses PostgreSQL konkuren; satu `pgx.Conn` tidak boleh dibagikan oleh handler paralel. Error database lengkap hanya masuk log internal, sedangkan response client harus stabil dan tidak membocorkan detail implementasi.
...
- Exception dependency hanya boleh memakai advisory ID, package, project, alasan, mitigasi, dan tanggal kedaluwarsa yang eksplisit serta disetujui pengguna; CI wajib tetap gagal untuk temuan lain dan setelah expiry. Exception kontingensi `image-size` yang diotorisasi untuk dua aplikasi mobile berakhir 7 Oktober 2026, tidak sedang terpakai pada lockfile Metro 0.84.5, dan mengikuti `docs/security/NPM_AUDIT_EXCEPTION_IMAGE_SIZE_2026-09-07.md`.
