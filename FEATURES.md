...
| Release candidate dan UAT bisnis terisolasi | `[x] Done` | v15 | `docs/runbook/RELEASE_CANDIDATE_UAT_V15.md`, `docs/release/RELEASE_MANIFEST_V15.md` | 19 pemeriksaan UAT lintas peran/domain dan UAT Peran kustom lulus tanpa fixture tersisa; create 201 serta perubahan status/arsip/pemulihan 204. `manage-realm` terbatas pada service account backend, bootstrap idempotent, restore RPO 0 detik/RTO 80,93 detik, dan Realtime RTO 15,04 detik. Production tidak berubah. |
...
| Fitur | Status | Versi | File/Area | Catatan |
|---|---|---|---|---|
| App Router setup | `[x] Done` | v13.1 | `apps/public-web-nextjs/` | Next.js 16, TypeScript, Tailwind v4, CSP nonce, PWA, dan shell Techwind clean-room aktif |
| Navigasi utama | `[x] Done` | v13.1 | `Navbar.tsx`, `Footer.tsx`, `PublicPageHero.tsx`, `BackToTop.tsx` | Beranda, Cabor, Venue, Jadwal, Klasemen, Jelajah, dan Berita tersedia konsisten pada desktop/mobile; shell menambah breadcrumb, skip-link, Escape pada menu, active-state, tema, footer, serta tombol kembali ke atas. |
| SEO metadata system | `[x] Done` | v13.5 | `app/**/metadata`, `layout.tsx`, `robots.ts`, `sitemap.ts` | Metadata/canonical, Open Graph detail Berita, WebSite dan NewsArticle JSON-LD ber-nonce, robots, serta sitemap detail Berita dinamis tersedia dan terverifikasi pada runtime lokal. |
...
| Beranda | `[x] Done` | v13.5 | `/`, `HomeCompetitionPulse.tsx`, `NewsSection.tsx` | Hero dinamis, pengantar, maskot, pusat informasi, ringkasan LiveScore/Jadwal/Medali faktual, Venue, panel berita sticky khusus desktop, Panduan Kota, dan CTA lulus visual acceptance 390/1440 px terang/gelap. Kandidat filter berita PORPROV bertag masih memerlukan gate runtime dan delivery tersendiri. |
| Cabor listing/detail | `[x] Done` | v13.3 | `/cabor`, `/cabor/[slug]`, `CaborDirectory.tsx` | Listing bergaya portfolio Techwind memiliki pencarian dan kategori dinamis; detail mempertahankan metadata, nomor tanding, venue, jadwal, slug publik, redirect UUID, serta Hero Image Media Library. |
...
| Venue & Maps | `[x] Done` | v13.5 | `/`, `/venue`, `/venue/[id]` | Listing/peta Techwind memiliki pencarian, filter kesiapan, fasilitas, kapasitas, cabor, rute, koordinat, jadwal, dan feedback peta aksesibel. Detail memilih rekomendasi per kelompok, memprioritaskan pin global, dan lulus acceptance responsif/tema final. |
| City Guide listing/search | `[x] Done` | v13.5 | `/city-guide`, `GET /city-guide-categories` | Public mengambil kategori aktif secara dinamis dengan fallback faktual dari data lokasi, serta mempertahankan pencarian, filter URL, pagination, rute, dan state lengkap. Pin default `Department Sports Lab` tetap diprioritaskan pada detail Venue; acceptance 390/1440 px terang/gelap lulus. |
...
| Berita Portal Depok | `[~] In Progress` | v13.5 | `/berita`, `/berita/[slug]`, `depok-news.ts`, `/api/berita/image` | Kandidat source memakai `GET /api/v1/find-posts-by-tags/PorprovJabar2026` sebagai satu-satunya daftar tayang, mengikuti pagination terikat, memblokir detail slug di luar tag, dan menautkan artikel lengkap ke sumber resmi. API bertag tidak menyediakan peringkat populer atau isi penuh, sehingga sidebar dinamai berita terkait dan ringkasan memakai fallback aman. Timeout, cache, sanitasi teks, proxy gambar same-origin, serta credential server-only tetap dipertahankan. Runtime production belum berubah sampai Git/release gate lulus. |
...
| Mobile dependency security gate | `[~] In Progress` | v0.2 | kedua aplikasi mobile, GitHub Actions, npm audit | Expo SDK 57 patch-compatible, lint, typecheck, dan export Web representatif tersedia. Kandidat patch `expo` 57.0.24, `@expo/ui`/`expo-constants` 57.0.19, serta `expo-router` 57.0.22 telah meluluskan `check:expo`, typecheck, lint, dan audit dependency lokal (nol Critical/High/Moderate) pada kedua aplikasi; CI SHA final tetap menjadi gate. Lockfile Metro 0.84.5 tidak lagi memuat `image-size`. CI tetap menolak semua Critical/High selain exception kontingensi dua advisory exact yang disetujui sampai 7 Oktober 2026; detail expiry/mitigasi berada di `docs/security/NPM_AUDIT_EXCEPTION_IMAGE_SIZE_2026-09-07.md`. |
...
| Media storage convergence | `[x] Done` | v0.5 | `master_data_uploads`, runtime migration | Sebanyak 374 metadata aset aktif terverifikasi dapat diakses HTTP 200 dari Gateway; file lokal legacy dipindahkan ke backup non-Git tanpa purge |
...
| Observability & visitor analytics | `[~] In Progress` | v0.4 | Prometheus, Grafana, Umami, API Gateway, Dashboard Admin | Prometheus memantau layanan teknis; Umami self-hosted mengumpulkan page views anonim same-origin melalui validator API Gateway yang membatasi Origin, website, hostname, tipe event, ukuran, URL/referrer, dan judul. Dashboard Admin menampilkan agregat ber-RBAC. Loki/Otel/alerting dan verifikasi retensi production masih bertahap. |
| Security scan dan edge hardening | `[~] In Progress` | v0.9 | ZAP, Nginx, API Gateway, GitHub Actions, npm, govulncheck | Baseline CSP/HSTS, COOP/COEP/CORP, CORS, canonical 404, sanitasi 5xx, font self-hosted, dependency gates, dan production fail-closed tersedia. Alert ZAP High 15 September 2026 pada `payload.title` kolektor analitik tidak membuktikan akses filesystem, tetapi boundary diperkeras: request kini divalidasi API Gateway dan `title=/collect` ditolak sebelum Umami; unit test, build image, `nginx -t`, secret scan, dan `govulncheck` lokal lulus. Aktivasi production dan ZAP konfirmasi mengikuti gate `DEPLOYMENT_VPS.md`; active/authenticated scan dan rotasi credential historis tetap diperlukan. |
