...
| Event hero | `[x] Done` | v0.3 | `apps/public-web-nextjs/src/components/HeroSection.tsx` | Hero 100 viewport, parallax 50%, periode resmi `7 - 23 November 2026`, countdown, dan reduced-motion fallback; CTA Jadwal/Venue pada hero telah dihapus sesuai keputusan konten 27 Juli 2026 |
...
| Venue & Maps | `[~] In Progress` | v0.7 | `/`, `/venue`, `/venue/[id]` | Listing live dan detail dengan fasilitas, kapasitas, cabor, rute, koordinat, jadwal, serta popup penanda peta berkontras AA pada light/dark tersedia. Bagian bawah Detail Venue menampilkan tepat satu rekomendasi terdekat per enam kelompok City Guide—Tempat Menginap, Pusat Perbelanjaan, Wisata Kuliner, Coffee Shop, Rumah Sakit, dan Lainnya—dengan foto, jarak Haversine, nama, alamat, serta tautan rute; peta interaktif lanjutan belum final. |
...
| Master data | `[x] Done` | v0.7 | cabor, nomor pertandingan, venue, kontingen, jadwal, City Guide | CRUD, pencarian, referensi, Media Selector, form Jadwal + Peserta A/B, serta City Guide. City Guide menyediakan URL Google Maps (Rute) opsional, label rute bernama, dan fallback koordinat otomatis. Semua tabel master data distandardisasi dengan pagination, sorting, rows-per-page, dan dark mode compatible. |
...
| Master Data Service | `[~] In Progress` | v0.7 | `services/master-data-service/` | CRUD serta soft delete/restore Cabor, Nomor, Kontingen, City Guide, dan Media tersedia; City Guide memakai pasangan latitude/longitude tervalidasi serta `map_route_url` Google Maps opsional dengan fallback koordinat pada migration v7, sedangkan immutable audit persistence/outbox dan RBAC granular belum selesai |
