...
| Security scan dan edge hardening | `[~] In Progress` | v0.9 | ZAP, Nginx, API Gateway, GitHub Actions, npm, govulncheck | Baseline CSP/HSTS, COOP/COEP/CORP, CORS, canonical 404, sanitasi 5xx, font self-hosted, dependency gates, dan production fail-closed tersedia. Alert ZAP High 15 September 2026 pada `payload.title` tidak membuktikan akses filesystem; validator API Gateway kini aktif production pada commit `ecbbb270`, payload sah `200` dan `title=/collect` ditolak `400`. ZAP pasif 21 September mencatat `0 FAIL`, tidak ada High, dan satu Medium SRI pada preload gambar same-origin yang masih perlu triase formal; active/authenticated scan dan rotasi credential historis tetap diperlukan. |
