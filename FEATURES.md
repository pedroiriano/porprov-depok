...
| Beranda | `[x] Done` | v13.5 | `/`, `HomeCompetitionPulse.tsx`, `NewsSection.tsx` | Hero dinamis, pengantar, maskot, pusat informasi, ringkasan LiveScore/Jadwal/Medali faktual, Venue, Berita Depok fail-safe dengan panel populer sticky khusus desktop, Panduan Kota, dan CTA lulus visual acceptance 390/1440 px terang/gelap. |
...
| Berita Portal Depok | `[x] Done` | v13.5 | `/berita`, `/berita/[slug]`, `depok-news.ts`, `/api/berita/image` | Feed terkini/populer terverifikasi melalui request GET server-only; adapter menerapkan timeout, cache, normalisasi defensif, teks tersanitasi, fallback aman, serta proxy gambar same-origin fail-closed. Detail mencari slug pada jendela feed karena kontrak bySlug/byTag belum tersedia; credential hanya berada pada `.env` lokal yang diabaikan Git. |
