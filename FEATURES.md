...
| Master Data Service | `[~] In Progress` | v0.8 | `services/master-data-service/` | CRUD serta soft delete/restore Cabor, Nomor, Kontingen, City Guide, Media, dan Hero tersedia; Hero migration v8 menerapkan status aktif tunggal dan referensi Media Library, sedangkan immutable audit persistence/outbox dan RBAC granular belum selesai |
...
| Nginx SSL | `[~] In Progress` | v0.6 | `infra/docker/nginx/`, `docker-compose.vps.yml` | Edge HTTPS intranet, Admin `/admin/`, issuer Keycloak HTTPS, generator sertifikat dan job deploy tahan reconnect tersedia; smoke test VPS dan distribusi CA masih berjalan |
