...
| Target Cuba Admin | `[~] In Progress` | v5.5C | `apps/admin-web-react/src/components/cuba/`, `/`, `/user-management`, `/master-data`, `/city-guide`, `/hero`, `/media`, `/livescore`, `/medals`, `/verifikasi`, `/audit-log`, `/profile`, `VITE_ADMIN_CUBA_PHASE_1` | Gate runtime v5.3 dan visual acceptance terautentikasi v5.4E pada 390/1440 light/dark lulus. v5.4A–v5.4D menyeragamkan Master Data, City Guide, dan Recycle Bin; v5.5A memigrasikan Hero/Media; v5.5B memigrasikan LiveScore/Medali/Verifikasi; v5.5C memigrasikan Dashboard/Audit/Profil. Seluruhnya clean-room tanpa source/aset vendor; flag aktif lokal dan nonaktif production |
...
| Master data | `[~] In Progress` | v5.6 | cabor, nomor pertandingan, venue, kontingen, jadwal, City Guide | CRUD, pencarian, referensi, Media Selector, form Jadwal + Peserta A/B, City Guide, modal/draft/riwayat, serta tabel Cuba tersedia. City Guide memakai pagination server-side dan validasi lokasi; gambar memakai Media Library. Implementasi lokal lulus, sedangkan deployment production tetap pending. |
| Media Library | `[x] Done` | v5.6 | `components/media/`, `components/cuba/AdminMediaGrid.tsx`, master-data-service | Galeri dan selector Cuba memakai pencarian debounce, cancellation, sorting, pagination server-side maksimal 100, KPI seluruh library, validasi upload, soft delete/restore, dan retensi file. Metadata v13 menyimpan checksum SHA-256, dimensi, serta actor; kebijakan purge fisik tetap TBD. |
...
| Autosave dan pemulihan draft | `[~] In Progress` | v5.6 | `ModalForm.tsx`, `lib/formDraftStorage.ts`, form domain Admin | Draft IndexedDB terisolasi berdasarkan OIDC subject, route, entity, dan versi form; retensi 7 hari, restore/discard, status autosave, visibility/unload guard, serta konfirmasi tutup tersedia lintas form utama. Password/token tidak disimpan. Sinkronisasi server dan resolusi konflik `409` tetap backlog lintas perangkat. |
...
| Draft recovery + generic revision history | `[~] In Progress` | Draft lokal aman dan revision history audit generik tersedia lintas form utama; server draft lintas perangkat dengan conflict `409` belum tersedia. |
| Media policy endpoint/checksum/variant | `[~] In Progress` | Policy endpoint, pipeline ≤3 MiB, checksum, dimensi, dan actor sudah aktif; derivative image variants/asynchronous optimization belum tersedia. |
