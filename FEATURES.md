...
| Media Library | `[x] Done` | v5.5A | `components/media/`, `components/cuba/AdminMediaGrid.tsx`, master-data-service | Upload/selector/URL relatif, soft delete metadata, penyembunyian delivery publik, retensi file, dan restore tetap utuh. Galeri clean-room Cuba menyediakan KPI, pencarian, sorting, jumlah data, pagination, aksi sentuh/keyboard, modal alasan arsip, dan Media Selector terpaginasikan; kebijakan purge tetap TBD |
...
| Autosave dan pemulihan draft | `[~] In Progress` | v5.1 | `/user-management`, `lib/userDraftStorage.ts` | Form pengguna menyimpan field non-password ke IndexedDB per OIDC subject selama maksimal 7 hari, menyediakan restore/discard/status dan unload guard; server draft, conflict `409`, serta adopsi lintas form belum tersedia |
...
| Draft recovery + generic revision history | `[~] In Progress` | Draft lokal aman tersedia pada form pengguna; server draft, adopsi lintas form, dan revision history generik belum ada |
