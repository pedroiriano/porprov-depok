...
| Target Cuba Admin | `[~] In Progress` | v11 | `apps/admin-web-react/`, `docs/uiux/` | Seluruh route utama memakai implementasi clean-room tanpa source/aset vendor. Tahap 11 menambah draf server, riwayat perbandingan, pagination operasional, media derivative, Kesehatan Integrasi, dan bahasa Indonesia natural; runtime lokal aktif, sedangkan production tetap menunggu deployment terpisah. |
...
| Root OpenAPI contract-first | `[~] In Progress` | `openapi/openapi.yaml` mencakup draf, kesehatan integrasi, media, dan lima daftar operasional berhalaman Tahap 11 serta diperiksa otomatis; endpoint lama lain dikonvergensikan bertahap. |
