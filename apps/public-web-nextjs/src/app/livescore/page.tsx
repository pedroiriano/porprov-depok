      ...
      {loading ? <div className="flex min-h-64 items-center justify-center" role="status"><Loader2 className="size-8 animate-spin text-primary-500" aria-hidden="true" /><span className="sr-only">Memuat LiveScore</span></div> : matches.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 px-6 py-16 text-center dark:border-slate-700"><Activity className="mx-auto size-12 text-primary-500" aria-hidden="true" /><h2 className="mt-5 text-xl font-black">Belum ada pertandingan aktif</h2><p className="mt-2 text-slate-500">LiveScore muncul otomatis setelah jadwal dipublikasikan.</p></div> : (
        <div className="grid gap-6 lg:grid-cols-[minmax(0,2fr)_320px]">
          <div className="space-y-5">{matches.map((match) => {
            const participants = [...match.participants].sort((first, second) => first.slot - second.slot).slice(0, 2);
            const participantsReady = participants.length === 2 && participants.every((participant) => participant.display_name && participant.display_name !== "Peserta menunggu konfirmasi");
            const live = participantsReady ? scores[match.id] : undefined;
            const participantRows = participants.length === 2
              ? participants.map((participant) => ({
                name: participant.display_name,
                context: participant.participant_type === "contingent" || participant.display_name === participant.kontingen_name ? "" : participant.kontingen_name,
              }))
              : [
                { name: "Peserta menunggu konfirmasi", context: "" },
                { name: "Lawan menunggu konfirmasi", context: "" },
              ];
            ...
            return <article key={match.id} className={`overflow-hidden rounded-2xl border bg-white shadow-sm transition duration-500 dark:bg-slate-900 ${live ? "border-red-300 dark:border-red-900" : "border-slate-200 dark:border-slate-800"} ${flashCard === match.id ? "scale-[1.01] shadow-lg ring-2 ring-amber-300" : ""}`}>
              <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/40"><div><p className="text-xs font-black uppercase tracking-wider text-primary-600 dark:text-primary-300">{match.caborName}</p><p className="mt-1 text-sm font-bold">{match.nomorTandingName} · {match.round}</p></div><span className={`rounded-full px-3 py-1 text-xs font-black uppercase tracking-wider ${live ? "bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-300" : "bg-slate-200 text-slate-600 dark:bg-slate-800 dark:text-slate-300"}`}>{participantsReady ? (live?.status || match.status) : "Menunggu peserta"}</span></header>
              <div className="space-y-4 p-5 sm:p-6">{!participantsReady && <div className="rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-100">Skor ditahan sampai susunan Peserta A dan B dikonfirmasi panitia.</div>}{participantRows.map((participant, index) => <div key={`${match.id}-${index}`} className="flex items-center justify-between gap-4"><div className="flex min-w-0 items-center gap-3"><span className="flex size-11 shrink-0 items-center justify-center rounded-full bg-primary-500/10 font-black text-primary-600">{participant.name.charAt(0)}</span><div className="min-w-0"><p className="truncate font-black">{participant.name}</p>{participant.context && <p className="truncate text-xs text-slate-500 dark:text-slate-400">{participant.context}</p>}</div></div><span className="text-4xl font-black tabular-nums">{scoreValues[index] ?? "–"}</span></div>)}<div className="flex flex-wrap justify-between gap-3 border-t border-slate-100 pt-4 text-xs text-slate-500 dark:border-slate-800"><span className="flex items-center gap-1.5"><Clock className="size-3.5" aria-hidden="true" />{match.matchDate ? new Date(match.matchDate).toLocaleString("id-ID", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" }) : "Waktu menyusul"}</span><span className="flex items-center gap-1.5"><MapPin className="size-3.5" aria-hidden="true" />{match.venueId ? <Link href={`/venue/${encodeURIComponent(match.venueId)}`} className="hover:text-primary-600">{match.venueName}</Link> : match.venueName}</span>{live?.sequence !== undefined && <span>Event #{live.sequence}{live.timestamp ? ` · ${new Date(live.timestamp).toLocaleTimeString("id-ID")}` : ""}</span>}</div></div>
            </article>;
          })}</div>
