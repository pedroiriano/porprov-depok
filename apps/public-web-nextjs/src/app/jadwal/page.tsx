    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-text-muted mt-2">Filter dan pantau jadwal dari 64 Cabang Olahraga.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative">
            ...
            <input 
              ...
              className="pl-10 pr-4 py-2 bg-background-surface border border-slate-200 dark:border-slate-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 w-full sm:w-64"
            />
          </div>
          ...
          <button 
            ...
            className={`flex-none flex flex-col items-center justify-center px-6 py-3 rounded-xl border transition-all ${
              idx === 2 
                ...
                : "bg-background-surface border-slate-200 dark:border-slate-800 hover:border-primary-300"
            }`}
          >
            <span className={`text-xs font-medium mb-1 ${idx === 2 ? "text-blue-100" : "text-text-muted"}`}>
              {idx === 2 ? "HARI INI" : "Rabu"}
            </span>
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="glass-card bg-background-surface border border-slate-200 dark:border-slate-800 overflow-hidden divide-y divide-slate-100 dark:divide-slate-800">
            {[1, 2, 3].map((i) => (
              <div key={i} className="p-4 sm:p-5 flex flex-col sm:flex-row gap-4 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                ...
                <div className="flex flex-row sm:flex-col items-center sm:items-start justify-between sm:justify-start gap-2 sm:w-24 shrink-0 border-b sm:border-b-0 border-slate-100 dark:border-slate-800 pb-3 sm:pb-0">
                  <div className="flex items-center gap-1.5 text-text-primary font-bold">
                    <Clock className="w-4 h-4 text-text-muted" /> 09:00
                  </div>
                  {i === 1 ? (
