...
export default function MasterData() {
  ...
  const fetchCabors = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API_BASE_URL}/master-data/cabors`);
      setCabors(res.data || []);
    } catch (error) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      await axios.post(`${API_BASE_URL}/master-data/cabors`, formData);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await axios.delete(`${API_BASE_URL}/master-data/cabors/${id}`);
      fetchCabors();
    } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-text-primary">Master Data: Cabang Olahraga</h2>
