...
function validateHeroForm(form: HeroFormState) {
  if (!form.title.trim()) return 'Judul Hero wajib diisi.';
  if (!form.description.trim()) return 'Isi Hero wajib diisi.';
  if (!form.background_image_url.trim()) return 'Pilih gambar Hero dari Media Library.';
  ...
  if (highlight && !form.title.toLocaleLowerCase('id-ID').includes(highlight)) {
    return 'Teks sorotan harus merupakan bagian dari judul Hero.';
  }
  return '';
}
...
export default function HeroManagement() {
  ...
  const [archiveReason, setArchiveReason] = useState('Diarsipkan melalui workspace Hero Utama');
  ...
  const saveMutation = useMutation({
    ...
    onError: (error) => setFormError(getApiErrorMessage(error, 'Gagal menyimpan Hero.')),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => {
      ...
      setFeedback(getApiErrorMessage(error, 'Gagal mengarsipkan Hero.'));
      setArchiveTarget(null);
    },
  });
  ...
  return (
    <section aria-labelledby="hero-management-title">
      <AdminPageHeader
        ...
        title="Hero Utama"
        description="Kelola pesan pembuka dan gambar latar Web Publik dengan satu Hero aktif sebagai sumber tayang canonical."
        actions={(
          <button type="button" onClick={openCreate} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 text-sm font-black text-white shadow-sm hover:bg-blue-700">
            <Plus className="size-4" aria-hidden="true" /> Tambah Hero
          </button>
        )}
      />
      <h1 id="hero-management-title" className="sr-only">Hero Utama</h1>

      <div className="mb-6 grid gap-3 sm:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-xl bg-blue-50 text-blue-600 dark:bg-blue-950/50 dark:text-blue-200"><Layers3 className="size-5" aria-hidden="true" /></span><div><p className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">Total Hero</p><p className="text-2xl font-black text-slate-950 dark:text-white">{heroes.length}</p></div></div></div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-xl bg-emerald-50 text-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-200"><CheckCircle2 className="size-5" aria-hidden="true" /></span><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">Hero aktif</p><p className="truncate text-base font-black text-slate-950 dark:text-white" title={activeHero?.title}>{activeHero?.title || 'Belum tersedia'}</p></div></div></div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-xl bg-amber-50 text-amber-700 dark:bg-amber-950/50 dark:text-amber-200"><FilePenLine className="size-5" aria-hidden="true" /></span><div><p className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">Draft</p><p className="text-2xl font-black text-slate-950 dark:text-white">{draftCount}</p></div></div></div>
      </div>

      {feedback && <div className="mb-4"><AdminAlert tone={feedbackTone}>{feedback}</AdminAlert></div>}

      {heroesQuery.isLoading ? (
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900"><AdminLoadingState label="Memuat Hero Utama..." /></div>
      ) : heroesQuery.isError ? (
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900"><AdminErrorState message={getApiErrorMessage(heroesQuery.error, 'Gagal memuat Hero.')} onRetry={() => void heroesQuery.refetch()} /></div>
      ) : heroes.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900"><AdminEmptyState icon={ImageIcon} title="Belum ada Hero" description="Tambahkan Hero pertama untuk mengganti konten fallback Web Publik." action={<button type="button" onClick={openCreate} className="min-h-11 rounded-xl bg-blue-600 px-4 text-sm font-black text-white hover:bg-blue-700">Tambah Hero pertama</button>} /></div>
      ) : (
              ...
              <article key={hero.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
                <div className="relative isolate flex min-h-72 items-end overflow-hidden p-5 sm:p-7">
                  ...
                  <div className="w-full">
                    <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-black uppercase tracking-wide ring-1 ${hero.is_active ? 'bg-emerald-400/20 text-emerald-100 ring-emerald-300/40' : 'bg-white/15 text-white ring-white/25'}`}>
                      {hero.is_active && <CheckCircle2 className="size-3.5" aria-hidden="true" />} {hero.is_active ? 'Aktif' : 'Draft'}
                    </span>
                ...
                <div className="flex flex-col gap-3 border-t border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between dark:border-slate-700">
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:flex">
                    ...
                    <button type="button" onClick={() => { setArchiveReason('Diarsipkan melalui workspace Hero Utama'); setArchiveTarget(hero); }} disabled={deleteMutation.isPending} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-red-200 px-4 text-sm font-bold text-red-700 hover:bg-red-50 disabled:opacity-50 dark:border-red-900 dark:text-red-300 dark:hover:bg-red-950/40"><Trash2 className="size-4" aria-hidden="true" /> Arsipkan</button>
                  </div>
                </div>
              </article>
            );
      ...
      <ModalForm isOpen={formOpen} onClose={() => !saveMutation.isPending && setFormOpen(false)} title={editing ? 'Edit Hero Utama' : 'Tambah Hero Utama'} onSubmit={submit} submitting={saveMutation.isPending} submitText={editing ? 'Simpan Perubahan' : 'Tambah Hero'} size="large" draft={{ entityId: editing?.id || 'new-hero', version: 'hero-v1', value: form, onRestore: setForm }}>
        ...
        <MediaInput label="Latar Belakang Gambar" required value={form.background_image_url} onClear={() => setForm({ ...form, background_image_url: '' })} onSelect={() => setMediaOpen(true)} placeholderText="Pilih dari Media Library" previewVariant="landscape" helpText="Gunakan gambar lanskap dengan subjek utama tetap terbaca pada layar ponsel maupun desktop." />
        <label className="flex min-h-11 cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/60">
          ...
          <span><span className="block font-black text-slate-900 dark:text-white">Tampilkan sebagai Hero aktif</span><span className="mt-1 block text-sm text-slate-600 dark:text-slate-300">Mengaktifkan Hero ini otomatis menonaktifkan Hero aktif sebelumnya.</span></span>
        </label>
      </ModalForm>
      ...
      <Modal isOpen={archiveTarget !== null} onClose={() => !deleteMutation.isPending && setArchiveTarget(null)} title="Arsipkan Hero" description="Hero dipindahkan ke Recycle Bin dan dapat dipulihkan sesuai kewenangan." closeDisabled={deleteMutation.isPending}>
        <form className="space-y-4 p-4 sm:p-6" onSubmit={(event) => { event.preventDefault(); if (archiveTarget) deleteMutation.mutate({ hero: archiveTarget, reason: archiveReason.trim() || 'Diarsipkan melalui workspace Hero Utama' }); }}>
          ...
          <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={() => setArchiveTarget(null)} disabled={deleteMutation.isPending} className="min-h-11 rounded-xl border border-slate-300 px-4 text-sm font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-50 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800">Batal</button><button type="submit" disabled={deleteMutation.isPending} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-red-600 px-4 text-sm font-black text-white hover:bg-red-700 disabled:opacity-50">{deleteMutation.isPending && <Loader2 className="size-4 animate-spin" aria-hidden="true" />} {deleteMutation.isPending ? 'Mengarsipkan...' : 'Arsipkan Hero'}</button></div>
        </form>
      </Modal>
    </section>
  );
