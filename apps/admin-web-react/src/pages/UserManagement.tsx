...
function validateUserForm(form: UserForm, editing: boolean): UserFormErrors {
  ...
  if (!form.role) errors.role = 'Pilih satu role pengguna.';
  if ((!editing || form.password) && (form.password.length < 12 || form.password.length > 128)) errors.password = 'Password harus terdiri dari 12–128 karakter.';
  return errors;
}
...
export default function UserManagement() {
  ...
  const fetchRoles = useCallback(async () => {
    ...
    try {
      ...
      if (roleList.length === 0) {
        ...
        setRolesError('Role dari Keycloak kosong; daftar role canonical sementara digunakan.');
      } else {
        setRoles(roleList);
      }
    } catch (error) {
      setRoles(FALLBACK_ROLES);
      setRolesError(getApiErrorMessage(error, 'Role Keycloak gagal dimuat; daftar role canonical sementara digunakan.'));
    } finally {
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(false);
    await fetchUsers();
    setOperationError(failed > 0 ? `${failed} pengguna gagal dinonaktifkan. Periksa Audit Log sebelum mencoba kembali.` : '');
    setOperationMessage(completed > 0 ? `${completed} pengguna berhasil dinonaktifkan dengan soft delete.` : '');
  };

  const columns = useMemo<Array<AdminDataTableColumn<User, UserSortKey>>>(() => [
    { key: 'username', label: 'Username', sortKey: 'username', render: (user) => <span className="font-black text-slate-900 dark:text-white">{user.username}</span> },
    ...
    { key: 'email', label: 'Email', sortKey: 'email', render: (user) => <span className="text-sm text-slate-600 dark:text-slate-200">{user.email}</span> },
    { key: 'role', label: 'Role', sortKey: 'role', render: (user) => <span className="inline-flex rounded-full border border-blue-200 bg-blue-50 px-2.5 py-1 text-xs font-black text-blue-700 dark:border-blue-800 dark:bg-blue-950/50 dark:text-blue-200">{user.role}</span> },
  ...
  return (
    <section aria-labelledby="user-management-title">
      <AdminPageHeader
        ...
        description="Kelola identitas dan role operator. Otorisasi akhir tetap diverifikasi oleh API Gateway dan Keycloak."
      ...
      <div className="mb-4 grid gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-xl bg-emerald-50 text-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-200"><ShieldCheck className="size-5" aria-hidden="true" /></span><div><p className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Role tersedia</p><p className="text-2xl font-black text-slate-950 dark:text-white">{availableRoles.length}</p></div></div></div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
        ...
        {!loading && !usersError && pagination.total > 0 && <TablePagination currentPage={pagination.page} totalPages={pagination.total_pages} totalItems={pagination.total} startItem={startItem} endItem={endItem} onPageChange={handleChangePage} itemLabel="pengguna" footerNote="pagination server-side" />}
      </div>

      <Modal isOpen={isModalOpen} onClose={closeEditor} closeDisabled={submitting} title={editingUser ? 'Edit pengguna' : 'Tambah pengguna'} description="Perubahan identitas dan role akan disinkronkan ke Keycloak dan audit trail." maxWidth="lg">
        <form onSubmit={handleSubmit} className="space-y-5 p-4 sm:p-6" noValidate>
          ...
          {availableDraft && (
            <AdminAlert tone="warning">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <span>Draft lokal {new Date(availableDraft.savedAt).toLocaleString('id-ID')} tersedia. Password tidak pernah disimpan.</span>
                <span className="flex shrink-0 flex-wrap gap-2">
                  ...
                  <button type="button" onClick={removeAvailableDraft} className="min-h-11 rounded-lg border border-yellow-700 px-3 text-xs font-black hover:bg-yellow-100 dark:border-yellow-300 dark:hover:bg-yellow-950">Buang draft</button>
                </span>
              </div>
            </AdminAlert>
          )}
          <TextInput label="Username" name="username" value={formData.username} onChange={(event) => updateField('username', event.target.value)} error={formErrors.username} autoComplete="username" required />
          <TextInput label="Email" type="email" name="email" value={formData.email} onChange={(event) => updateField('email', event.target.value)} error={formErrors.email} autoComplete="email" required />
          ...
          {rolesLoading ? <div className="flex min-h-11 items-center gap-2 text-sm font-bold text-slate-500 dark:text-slate-300" role="status"><Loader2 className="size-4 animate-spin" aria-hidden="true" /> Memuat role...</div> : <><SelectInput label="Role" name="role" value={formData.role} onChange={(event) => updateField('role', event.target.value)} options={availableRoles.map((role) => ({ value: role, label: role }))} error={formErrors.role} required />{rolesError && <AdminAlert tone="warning">{rolesError}</AdminAlert>}</>}
          <div className="relative"><KeyRound className="pointer-events-none absolute right-3 top-10 size-4 text-slate-400" aria-hidden="true" /><TextInput label={editingUser ? 'Password baru' : 'Password awal'} type="password" name="password" value={formData.password} onChange={(event) => updateField('password', event.target.value)} error={formErrors.password} helpText={editingUser ? 'Kosongkan jika password tidak ingin diubah.' : 'Minimal 12 karakter; jangan menggunakan password sementara yang mudah ditebak.'} autoComplete="new-password" required={!editingUser} /></div>
          <p className="text-xs font-semibold text-slate-500 dark:text-slate-300" role="status" aria-live="polite">
            {draftStatus === 'saving' && 'Menyimpan draft aman...'}
            {draftStatus === 'saved' && 'Draft lokal tersimpan selama maksimal 7 hari. Password tidak disimpan.'}
            {draftStatus === 'idle' && 'Perubahan non-password akan disimpan otomatis di perangkat ini.'}
            {draftStatus === 'error' && 'Draft lokal gagal disimpan. Jangan tutup atau muat ulang halaman sebelum mencoba kembali.'}
          </p>
      ...
      <Modal isOpen={closeConfirmationOpen} onClose={() => setCloseConfirmationOpen(false)} title="Simpan perubahan sebagai draft?" description="Form memiliki perubahan yang belum dikirim ke server." maxWidth="md">
        <div className="space-y-5 p-4 sm:p-6">
          <AdminAlert tone="warning">Username, email, nama, dan role dapat dipulihkan hingga 7 hari. Demi keamanan, password tidak pernah dimasukkan ke draft.</AdminAlert>
