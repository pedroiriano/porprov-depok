...
const scoreStatuses = ['Belum Mulai', 'Berlangsung', 'Istirahat', 'Selesai', 'Official'];
...
export default function LiveScoreCenter() {
  ...
  const matchOptions = useMemo(() => matches.map((match) => {
    const names = [...(match.participants || [])].sort((a, b) => a.slot - b.slot).map((participant) => participant.display_name).join(' vs ');
    return { id: match.id, label: `${match.cabor_name} · ${match.nomor_tanding_name} · ${names || 'peserta belum lengkap'}` };
  }), [matches]);

  return (
    <div className="flex flex-col gap-6">
      <AdminPageHeader eyebrow="Official scoring workspace" title="LiveScore Center" description="Kelola skor dan status pertandingan melalui revisi append-only, audit trail, serta transactional outbox. Susunan peserta tetap dikelola dari Jadwal Pertandingan." actions={<><span className={`inline-flex min-h-11 items-center gap-2 rounded-xl border px-4 text-sm font-black ${connected ? 'border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-200' : 'border-amber-200 bg-amber-50 text-amber-900 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-100'}`} role="status"><Radio className={`size-4 ${connected ? 'animate-pulse' : ''}`} aria-hidden="true" />{connected ? 'Private SSE aktif' : 'Menghubungkan SSE'}</span><button type="button" onClick={() => void loadData()} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 text-sm font-black text-slate-700 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800"><RefreshCw className="size-4" aria-hidden="true" />Perbarui</button></>} />
      {feedback && <AdminAlert tone={feedback.type === 'error' ? 'danger' : 'success'}>{feedback.message}</AdminAlert>}

      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="Ringkasan LiveScore">
      ...
      {loading ? <div className="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900"><AdminLoadingState label="Memuat LiveScore..." /></div> : matches.length === 0 ? <div className="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900"><AdminEmptyState icon={Trophy} title="Belum ada pertandingan" description="Susun pertandingan dan Peserta A/B pada Jadwal Pertandingan sebelum membuka input skor." action={<a href={`${import.meta.env.BASE_URL}master-data?tab=jadwal`} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-blue-600 px-4 text-sm font-black text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900">Buka Jadwal Pertandingan</a>} /></div> : (
        <div className="grid gap-6 xl:grid-cols-[minmax(320px,0.85fr)_minmax(0,1.5fr)]">
          <form onSubmit={submit} className="h-fit rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6 dark:border-slate-700 dark:bg-slate-900">
            <div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-blue-50 text-blue-600 dark:bg-blue-950/50 dark:text-blue-200"><Send className="size-5" aria-hidden="true" /></span><div><h2 className="font-black text-slate-950 dark:text-white">Input skor resmi</h2><p className="text-xs text-slate-500 dark:text-slate-300">Satu update menghasilkan revisi baru.</p></div></div>
            <div className="mt-4"><AdminAlert tone="warning"><strong>Alur resmi:</strong> workspace ini hanya mencatat skor, status, dan koreksi. Peserta tidak dapat diubah dari sini.</AdminAlert></div>
            ...
            {selectedMatch && <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm dark:border-slate-700 dark:bg-slate-800/70"><p className="font-black text-slate-900 dark:text-white">{selectedParticipants.map((item) => item.display_name).join(' vs ') || 'Peserta belum diatur'}</p><p className="mt-1 text-slate-500 dark:text-slate-300">{selectedMatch.venue_name} · {selectedMatch.round}</p></div>}
          ...
          <div className="space-y-6">
            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6 dark:border-slate-700 dark:bg-slate-900">
              <div className="flex items-center justify-between gap-3"><h2 className="flex items-center gap-2 font-black text-slate-950 dark:text-white"><History className="size-5 text-blue-600 dark:text-blue-300" aria-hidden="true" />Riwayat revisi</h2><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-600 dark:bg-slate-800 dark:text-slate-200">{history.length} revisi</span></div>
              {history.length === 0 ? <AdminEmptyState icon={History} title="Belum ada revisi" description="Skor pertama pertandingan ini belum disimpan." /> : <div className="mt-4 space-y-3">{history.map((item) => <article key={item.revisionId} className="rounded-xl border border-slate-200 p-4 dark:border-slate-700"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-black text-slate-950 dark:text-white">Revisi #{item.revisionNumber} · {item.scoreA}—{item.scoreB}</p><span className="rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-xs font-black text-blue-700 dark:border-blue-800 dark:bg-blue-950/50 dark:text-blue-200">{item.status}</span></div><p className="mt-2 break-words text-xs text-slate-500 dark:text-slate-300">{new Date(item.timestamp).toLocaleString('id-ID')} · actor {item.actor}</p>{item.correctionReason && <p className="mt-2 rounded-lg bg-amber-50 p-2 text-sm text-amber-900 dark:bg-amber-950/40 dark:text-amber-100">Koreksi: {item.correctionReason}</p>}</article>)}</div>}
            </section>
            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6 dark:border-slate-700 dark:bg-slate-900">
              <div className="flex items-center justify-between gap-3"><h2 className="flex items-center gap-2 font-black text-slate-950 dark:text-white"><Activity className="size-5 text-blue-600 dark:text-blue-300" aria-hidden="true" />Private event stream</h2><span className={`size-2.5 rounded-full ${connected ? 'bg-emerald-500' : 'bg-amber-500'}`} aria-hidden="true" /></div>
              {events.length === 0 ? <AdminEmptyState icon={Radio} title="Belum ada event baru" description="Event LiveScore privat pada sesi ini akan muncul secara realtime." /> : <div className="mt-4 max-h-80 space-y-2 overflow-auto rounded-xl bg-slate-950 p-3">{events.map((item, index) => <div key={`${item.eventId}-${index}`} className="break-all rounded-lg border border-slate-800 bg-slate-900 p-3 font-mono text-xs text-emerald-300">{item.eventType} · match {item.matchId} · sequence {item.sequence}</div>)}</div>}
            </section>
          </div>
        </div>
      )}
