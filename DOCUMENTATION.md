...
- Navigasi utama Public memakai urutan Beranda, Cabor, Venue, Jadwal, Klasemen, Jelajah, dan Berita. Entri Jelajah mengarah ke `/city-guide`, sedangkan Berita menuju `/berita`; keduanya menutup menu mobile setelah aktivasi dan memakai active-state/`aria-current`. Shell juga menyediakan skip-link, breadcrumb halaman dalam, tombol tema, footer, serta tombol kembali ke atas.

- Gaya informasi: cepat, padat, SEO-friendly, sports-event storytelling.
- Halaman utama: Hero dinamis, hitung mundur, CTA, pusat informasi, ringkasan pertandingan aktif/berikutnya, tiga besar klasemen, cabor, venue, Berita Depok, maskot, dan Panduan Kota.
...
- Tahap 13.4 mengganti daftar kategori Jelajah publik yang statis dengan `GET /api/v1/master-data/city-guide-categories`. Hanya slug aktif yang menjadi filter URL; bila endpoint kategori gagal, Public membentuk fallback dari kategori lokasi yang sudah berhasil dimuat tanpa mengarang data baru.
- Integrasi Portal Berita Depok berada sepenuhnya pada Server Components melalui `BERITA_DEPOK_BASE_URL` dan `BERITA_DEPOK_API_KEY`; credential tidak memakai awalan `NEXT_PUBLIC_`. Kandidat filter PORPROV memakai `GET /api/v1/find-posts-by-tags/PorprovJabar2026?page=N` sebagai sumber tunggal untuk Beranda, arsip, detail, dan sitemap. Respons bertipe paginator dengan lima artikel per halaman, `next_page_url`, `thumb_url`, dan `caption`; URL halaman berikutnya tidak diikuti mentah, melainkan nomor halaman dibentuk sendiri dan dibatasi. `caption` adalah keterangan foto, bukan isi artikel, sehingga ringkasan kosong memakai fallback aman dan halaman detail menautkan sumber resmi. Endpoint ini tidak memberi peringkat populer; sidebar hanya menampilkan berita terkait. Detail opsional melalui `BERITA_DEPOK_DETAIL_PATH_TEMPLATE` baru dibaca setelah slug terbukti termasuk tag. Konten eksternal dirender sebagai teks tersanitasi. Gambar HTTPS melewati `/api/berita/image` dengan host `BERITA_DEPOK_IMAGE_HOSTS`, pembatasan MIME/ukuran, larangan redirect, dan timeout sehingga CSP/COEP tidak diperlebar. Kegagalan sumber tidak boleh menggagalkan Beranda. Status production mengikuti release gate terpisah.
- Finalisasi lokal Fase 13.5 pada 8 September 2026 meluluskan seluruh 44 kombinasi acceptance visual: 11 rute representatif pada lebar 390 dan 1440 piksel dalam tema terang/gelap, tanpa overflow horizontal, gambar rusak, atau struktur `main`/`h1` ganda. Navigasi mobile, Escape, skip-link, fokus keyboard, metadata canonical, WebSite/NewsArticle JSON-LD ber-nonce, sitemap Berita dinamis, manifest, dan service worker juga terverifikasi. Runtime canonical tetap Docker root; perubahan belum dikirim ke Git.
- Hardening image Fase 13.5 memakai instalasi deterministik `npm ci`, memperbarui paket keamanan Alpine saat build, dan membuang npm/Yarn/Corepack dari runner Public karena tidak dibutuhkan saat runtime. Image Public dan Nginx lulus Trivy untuk Critical/High yang memiliki perbaikan; Nginx lulus `nginx -t`, security-header suite, serta ZAP baseline pasif HTTPS dengan nol temuan gagal.
