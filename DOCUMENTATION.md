...
- Integrasi Portal Berita Depok berada sepenuhnya pada Server Components melalui `BERITA_DEPOK_BASE_URL` dan `BERITA_DEPOK_API_KEY`; credential tidak memakai awalan `NEXT_PUBLIC_`. Endpoint terdokumentasi saat ini adalah `GET /api/v1/latest/{count}` dan `GET /api/v1/popular/{count}`. Karena koleksi belum mendefinisikan URL bySlug/byTag atau contoh respons, detail mencari slug pada feed terbatas dan dapat memakai `BERITA_DEPOK_DETAIL_PATH_TEMPLATE` hanya setelah kontrak resmi diverifikasi. Konten eksternal dirender sebagai teks tersanitasi. Gambar HTTPS melewati `/api/berita/image` dengan host `BERITA_DEPOK_IMAGE_HOSTS`, pembatasan MIME/ukuran, larangan redirect, dan timeout sehingga CSP/COEP tidak diperlebar. Kegagalan sumber tidak boleh menggagalkan Beranda.
